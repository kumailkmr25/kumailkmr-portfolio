...
export default function CaseStudiesPage() {
 ...
 <div className="relative h-full bg-background rounded-[31px] p-8 md:p-12 flex flex-col lg:flex-row gap-12 border border-white/5 group-hover:border-primary-border transition-colors duration-500 z-10 overflow-hidden">
 ...
 <div className="w-6 h-6 rounded-full bg-[#111111] border border-white/20 text-foreground-muted flex items-center justify-center text-xs font-bold flex-shrink-0 group-hover:border-primary-border group-hover:text-primary-light transition-colors">
 {idx + 1}
 </div>
 <p className="text-sm font-medium text-foreground-muted leading-relaxed pt-0.5">
 {step}
 </p>
 </div>
 ))}
 ...
 {cs.tags.map((tag) => (
 <span key={tag} className="text-xs text-foreground-muted font-medium tracking-wide uppercase px-3 py-1 rounded-full border border-white/10 bg-white/5">
 {tag}
 </span>
 ))}
 </div>
 ...
 <p className="text-foreground-muted mb-8 max-w-xl mx-auto">Let&apos;s design a custom architecture tailored to your specific bottlenecks.</p>
