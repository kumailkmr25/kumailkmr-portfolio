...
export function NotificationCenter() {
 ...
 className="absolute top-4 right-4 text-foreground-muted hover:text-foreground dark:hover:text-white transition-colors p-1"
 >
 <X className="w-4 h-4" />
 </button>
 )}
 ...
 onClick={() => setActiveToast(null)}
 className="flex-1 bg-[#09090b] dark:bg-white text-white dark:text-foreground text-center py-3 text-sm font-semibold rounded-lg hover:opacity-90 transition-opacity"
 >
 {activeToast.ctaText}
 </Link>
 )}
 ...
 target="_blank"
 onClick={() => setActiveToast(null)}
 className="flex-1 bg-surface text-foreground text-center py-3 text-sm font-semibold rounded-lg hover:bg-black/5 dark:hover:bg-white/5 transition-colors border border-border "
 >
 Book Session
 </Link>
 </div>
 ...
 <button 
 onClick={() => setActiveToast(null)}
 className="text-foreground-muted hover:text-foreground dark:hover:text-white transition-colors flex items-center gap-2 text-sm font-medium"
 >
 ...
 className="absolute top-full mt-3 right-0 w-80 md:w-96 bg-white dark:bg-background border border-border-strong rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[500px] z-50 origin-top-right"
 >
 {/* Header */}
 <div className="p-4 border-b border-border flex items-center justify-between bg-surface/50 /50">
 ...
 <button 
 onClick={toggleSound}
 className="p-2 hover:bg-black/5 dark:hover:bg-white/5 rounded-full transition-colors text-foreground-muted"
 ...
 <div 
 key={notif.id}
 className={`p-4 rounded-xl border ${!notif.read ? 'bg-primary/5 border-primary/20' : 'bg-surface border-border '}`}
 >
 <h4 className="font-bold text-foreground text-sm mb-1">{notif.title}</h4>
 <p className="text-xs text-foreground-muted mb-3 leading-relaxed">{notif.message}</p>
 {notif.ctaHref && (
 <Link 
 href={notif.ctaHref}
