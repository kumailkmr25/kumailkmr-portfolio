        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {reasons.map((reason, index) => (
            <motion.div
              ...
              className="flex items-start gap-4 p-6 bg-white dark:bg-[#1a1a1a] rounded-2xl shadow-sm border border-black/5 dark:border-white/5"
            >
              <CheckCircle2 className="w-6 h-6 text-primary flex-shrink-0 mt-0.5" />
              <div>
                <h4 className="text-lg font-bold text-[#09090b] dark:text-white mb-2">{reason.title}</h4>
                <p className="text-[#71717a] text-sm leading-relaxed">{reason.desc}</p>
              </div>
            </motion.div>
          ))}
        </div>
