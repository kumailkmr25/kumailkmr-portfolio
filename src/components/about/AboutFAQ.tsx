...
export function AboutFAQ() {
 ...
 <div 
 key={index}
 className={`border rounded-2xl overflow-hidden transition-colors ${isOpen ? 'border-primary/50 bg-surface/50 ' : 'border-border-strong bg-transparent'}`}
 >
 <button
 ...
 <ChevronDown className={`w-5 h-5 text-foreground-muted transition-transform duration-300 ${isOpen ? 'rotate-180 text-primary' : ''}`} />
 </button>
 ...
 >
 <div className="px-6 pb-6 pt-0 text-foreground-muted text-sm leading-relaxed border-t border-border pt-4">
 {faq.answer}
 </div>
 </motion.div>
 )}
