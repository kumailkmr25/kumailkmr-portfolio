    ...
    <>
      <main className="bg-[#f4f4f5] dark:bg-[#111111] min-h-screen">
        <section className="section-py section-container pt-32 max-w-5xl">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                  i === 0
                    ...
                    : "bg-white dark:bg-[#0a0a0a] text-[#71717a] border border-black/10 dark:border-white/10 hover:border-[#09090b] dark:hover:border-white"
                }`}
              >
                {category}
              </button>
          ...
          <div className="mt-16 grid grid-cols-1 md:grid-cols-2 gap-8">
            {articles.map((article, i) => (
              <motion.div
                ...
                className="premium-card p-8 bg-white dark:bg-[#0a0a0a] flex flex-col group"
              >
                ...
                <h3 className="text-xl font-bold text-[#09090b] dark:text-white mb-3 group-hover:text-primary dark:group-hover:text-primary-light transition-colors">
                  {article.title}
                </h3>
                <p className="text-[#71717a] leading-relaxed mb-8 flex-1">
                  {article.excerpt}
                </p>

                <div className="flex items-center justify-between pt-6 border-t border-black/5 dark:border-white/5">
                  <span className="text-sm font-medium text-[#09090b] dark:text-[#a1a1aa]">
                    {article.date}
                  </span>
                  <Link
                    href={article.slug}
          ...
          <div className="mt-20 p-8 rounded-2xl border border-black/10 dark:border-white/10 bg-white dark:bg-[#0a0a0a] text-center max-w-2xl mx-auto flex flex-col items-center">
            ...
            <p className="text-[#71717a] mb-6">Join a private list of founders and operational leaders receiving bi-weekly essays on business automation.</p>
            <form onSubmit={handleSubscribe} className="flex w-full max-w-md gap-2">
              <input 
                ...
                required
                className="flex-1 rounded-xl border border-black/10 dark:border-white/10 px-4 py-2 bg-transparent focus:outline-none focus:border-primary transition-colors"
              />
