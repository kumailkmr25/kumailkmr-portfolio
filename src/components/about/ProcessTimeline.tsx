...
export function ProcessTimeline() {
  ...
  return (
    <section className="py-24 bg-[#f4f4f5] dark:bg-[#111111]">
          ...
          <div className="space-y-6">
            {processSteps.map((step, index) => {
              ...
              return (
                ...
                >
                  {/* Timeline Dot */}
                  <div className="absolute left-8 md:left-1/2 w-4 h-4 rounded-full bg-white dark:bg-[#0a0a0a] border-2 border-primary -translate-x-1/2 z-10 shadow-[0_0_10px_rgba(16,185,129,0.3)]" />
                  ...
                  <div className={`w-full md:w-1/2 ${isEven ? 'md:pr-12 pl-16 md:pl-0 text-left md:text-right' : 'md:pl-12 pl-16 text-left'}`}>
                    <motion.div 
                      ...
                      className={`p-6 rounded-2xl border transition-all cursor-pointer ${isActive ? 'bg-white dark:bg-[#1a1a1a] border-primary/40 shadow-lg' : 'bg-transparent border-transparent hover:bg-white/50 dark:hover:bg-[#1a1a1a]/50'}`}
                    >
                      <div className={`flex items-center gap-4 mb-2 ${isEven ? 'md:flex-row-reverse flex-row' : 'flex-row'}`}>
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 transition-colors ${isActive ? 'bg-primary text-white' : 'bg-black/5 dark:bg-white/5 text-[#71717a]'}`}>
                          {step.icon}
                        </div>
                        <h3 className="text-lg font-bold text-[#09090b] dark:text-white font-sora">
                          {step.title}
                        </h3>
                      </div>
                      ...
                      <AnimatePresence>
                        {isActive && (
                          <motion.p
                            ...
                            className="text-[#71717a] text-sm leading-relaxed overflow-hidden"
                          >
                            {step.desc}
                          </motion.p>
                        )}
